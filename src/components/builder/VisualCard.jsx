...
export default function VisualCard({ visual }) {
  ...
  const filteredRows = useMemo(() => applyGlobalFilters(rawData, filters), [rawData, filters])
  ...
  const chartData = useMemo(
    () =>
      buildVisualData({
        ...
        sortField: visual.sortField || visual.xField,
  ...
  const renderChart = () => {
    if (!visual.xField || !visual.yField) {
      return (
              ...
              <tr>
                {chartData[0] &&
                  Object.keys(chartData[0]).map((key) => (
                    <th key={key} className="border-b border-slate-200 px-4 py-3 text-left font-semibold text-slate-700">
                      {key}
                    </th>
                  ))}
              </tr>
    ...
    if (visual.chartType === 'kpi') {
      const total = chartData.reduce((sum, item) => {
        if ('value' in item) return sum + Number(item.value || 0)
        return (
          sum +
          Object.keys(item)
            .filter((k) => k !== 'x')
      ...
      return (
        <div className="flex h-[260px] flex-col justify-center rounded-3xl border border-slate-200 bg-gradient-to-br from-white to-slate-50 p-8">
          <div className="text-sm text-slate-500">{visual.yField}</div>
          <div className="mt-2 text-4xl font-bold tracking-tight text-slate-900">{total.toLocaleString()}</div>
          <div className="mt-2 text-sm text-slate-500">Aggregation: {visual.aggregation}</div>
        </div>
      )
    }

    if (visual.chartType === 'pie' || visual.chartType === 'donut') {
      const pieData = chartData.map((d) => ({ name: d.x, value: d.value ?? 0 }))
      return (
        ...
        <div className="h-[320px]">
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={chartData}>
              ...
              <Tooltip />
              <Legend />
              {legendKeys.length ? legendKeys.map((k) => <Line key={k} type="monotone" dataKey={k} />) : <Line type="monotone" dataKey="value" />}
            </LineChart>
          </ResponsiveContainer>
        </div>
      )
        ...
        <div className="h-[320px]">
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={chartData}>
              ...
              <Tooltip />
              <Legend />
              {legendKeys.length ? legendKeys.map((k) => <Area key={k} type="monotone" dataKey={k} />) : <Area type="monotone" dataKey="value" />}
            </AreaChart>
          </ResponsiveContainer>
        </div>
      )
      ...
      <div className="h-[320px]">
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={chartData}>
            ...
            <Legend />
            {legendKeys.length ? (
              legendKeys.map((k) => <Bar key={k} dataKey={k} stackId={visual.chartType === 'stackedBar' ? 'a' : undefined} />)
            ) : (
              <Bar dataKey="value" />
            )}
          </BarChart>
  ...
  return (
    ...
    >
      ...
      <div className="mb-4 grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-4">
        <DropZone label="X Axis" value={visual.xField} onDropField={(field) => assignFieldToVisual({ visualId: visual.id, zone: 'xField', field })} />
        <DropZone label="Y Axis" value={visual.yField} onDropField={(field) => assignFieldToVisual({ visualId: visual.id, zone: 'yField', field })} />
        <DropZone label="Legend" value={visual.legendField} onDropField={(field) => assignFieldToVisual({ visualId: visual.id, zone: 'legendField', field })} />
        <DropZone label="Tooltip" value={visual.tooltipFields} onDropField={(field) => assignFieldToVisual({ visualId: visual.id, zone: 'tooltipFields', field })} />
      </div>
